...
public class VideoService {
    ...
    public List<VideoEntity> listAllMyVideo(){
        List<VideoEntity>list=new ArrayList<VideoEntity>();
        VideoEntity videoEntity;
        List<Map<String, Object>> lMaps=videoRecordsDAO.queryByUsername(SecurityContextHolder.getContext().getAuthentication().getName());
        for(int i=0;i<=lMaps.size()-1;i++){
            videoEntity=new VideoEntity(lMaps.get(i));
            list.add(videoEntity);
        ...
        if(!file.isEmpty()) {
            ...
            if (type != null) {
                if ("MP4".equals(type.toUpperCase())) {//全部验证完毕，开始注入
                    ...
                    String uid=userDAO.queryByUsername(SecurityContextHolder.getContext().getAuthentication().getName()).get(0).get("uid").toString();
